  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 flex items-center justify-between px-4 md:px-6 h-[60px] bg-bg/90 backdrop-blur-2xl border-b border-border">
      <div className="flex items-center gap-3">
        ...
        <button onClick={toggleSidebar} className="lg:hidden flex flex-col gap-1 p-1.5 cursor-pointer border-none bg-transparent" aria-label="Toggle sidebar">
          <span className="w-4.5 h-0.5 bg-muted" />
          <span className="w-4.5 h-0.5 bg-muted" />
          <span className="w-4.5 h-0.5 bg-muted" />
        </button>
