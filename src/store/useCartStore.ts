...
import { CustomerProfile, logoutCustomer } from '@/services/customerAuthService';
  ...
  persist(
    (set, get) => ({
      ...
      logout: async () => {
        ...
        // Clear customer, set authenticated false, and empty cart items but keep orderHistory visible
        set({
          customer: null,
          isAuthenticated: false,
          items: [],
      ...
      initializeSession: async (shopSlug, existingSessionId) => {
        ...
        try {
          ...
          set({
            ...
            items: activeSession.cart.length > 0 ? activeSession.cart : get().items,
            orderHistory: activeSession.orderHistory.length > 0 ? activeSession.orderHistory : get().orderHistory,
            tableNumber: activeSession.orderHistory[0]?.table.replace('Table ', '') || get().tableNumber || '',
            loading: false
          });
          ...
          set({
            ...
            items: fallbackSession.cart.length > 0 ? fallbackSession.cart : get().items,
            orderHistory: fallbackSession.orderHistory.length > 0 ? fallbackSession.orderHistory : get().orderHistory,
            loading: false
          });
      ...
      createOrder: async (orderData) => {
        ...
        try {
          ...
          // Save customer_order to database if user is logged in
          const customer = get().customer;
          if (customer) {
            try {
              await (supabase as any).from('customer_orders').insert({
                order_id: newDbOrder.id,
                customer_id: customer.customerId,
                shop_slug: session.shopSlug,
                items: dbItems,
                total: orderData.total,
                payment_id: orderData.paymentId || null,
                payment_method: orderData.paymentMethod || 'cash',
                status: 'pending',
                delivery_address: orderData.table,
              });
              ...
              console.warn('Failed to log order to customer profile:', custErr);
            }
          }
